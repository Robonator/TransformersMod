...
public class EntityLaserBeam extends EntityThrowable implements IEntityAdditionalSpawnData
{
        ...
        if (this.isEntityAlive())
        {
            if (worldObj.isRemote)
            {
                for (int i = 0; i < 5; ++i)
                {
                    ...
                    worldObj.spawnParticle(EnumParticleTypes.REDSTONE, posX + f, posY + 0.15F, posZ + f, blue ? -1.0D : 0.0D, 0.0D, blue ? 1.0D : 0.0D);
                }
            }
    ...
    protected void onImpact(MovingObjectPosition mop)
    {
        ...
        {
            if (worldObj.getBlockState(mop.getBlockPos()).getBlock().isOpaqueCube())
            {
                this.setDead();
            }
        }
