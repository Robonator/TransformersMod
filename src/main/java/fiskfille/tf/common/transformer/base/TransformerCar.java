...
public abstract class TransformerCar extends Transformer
{
    ...
    public void doNitroParticles(EntityPlayer player)
    {
        for (int i = 0; i < 4; ++i)
        {
            ...
            player.worldObj.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, side.xCoord, side.yCoord + 0.35F, side.zCoord, rand.nextFloat() / 20, rand.nextFloat() / 20, rand.nextFloat() / 20);
        }
        ...
        for (int i = 0; i < 10; ++i)
        {
            Vec3 side = NitroParticleHandler.getBackSideCoords(player, 0.15F, i < 2, -0.9, false);
            Random rand = new Random();
            player.worldObj.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, side.xCoord, side.yCoord + 0.35F, side.zCoord, rand.nextFloat() / 10, rand.nextFloat() / 10 + 0.05F, rand.nextFloat() / 10);
        }
    }
}
